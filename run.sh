...
ENDPOINT=https://wb-hiring-challenge-ratings.azurewebsites.net/
HOST=localhost
PORT=9876
TEST_DATA=df_test.csv
TRAIN_DATA=df_train.csv
MOVIES_DATA=df_movies.csv
...
function clean_all() {
  ...
  if [[ -f $MOVIES_DATA ]]; then
    log "Removing previous $MOVIES_DATA...\\n"
    rm "$MOVIES_DATA"
    log " --> Done! \\n\\n"
  fi
  ...
  if [[ ! -f $TRAIN_DATA ]] || [[ ! -f $TEST_DATA ]]; then
    log "Downloading datasets...\\n"
    {
      ...
      curl \
        --silent \
        -request GET \
        ${ENDPOINT}train_url > $TRAIN_DATA

      curl \
        --silent \
        -request GET \
        ${ENDPOINT}test_url > $TEST_DATA

      curl \
        --silent \
        -request GET \
        ${ENDPOINT}movies_url > $MOVIES_DATA
    } && {
      log " --> Done! \\n\\n"
    } || { # catch
...
function train_model() {
  log "Training: POST $TRAIN_DATA to ${HOST}:${PORT}/ratings/train...\\n"
  { # try
    curl \
      --fail \
      --request POST \
...
function compute_predictions() {
  log "Predicting: POST $TEST_DATA to ${HOST}:${PORT}/ratings/predict > $SUBMISSION...\\n"
  { # try
    curl \
      --fail \
      --silent \
